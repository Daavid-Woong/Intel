...
RUN echo 'Daavid' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/Daavid.sh

RUN echo 'cd ' >>/Daavid.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Daavid.sh

RUN echo 'cd /noVNC-1.2.0' >>/Daavid.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Daavid.sh

RUN chmod 755 /Daavid.sh
